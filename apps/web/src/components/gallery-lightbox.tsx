...
export function GalleryLightbox({ publicIds, title }: GalleryLightboxProps) {
 ...
 className="block w-full cursor-zoom-in group focus:outline-none focus:ring-2 focus:ring-brand-negro/40 "
 ...
 alt={`${title} — ${i + 1}`}
 priority={i === 0}
 className="aspect-[16/10] bg-brand-crema overflow-hidden"
 />
 </button>
 ))}
 </div>
 ...
 priority
 className="w-auto max-w-[92vw] max-h-[85vh] overflow-hidden"
 />
 ) : (
 <img
